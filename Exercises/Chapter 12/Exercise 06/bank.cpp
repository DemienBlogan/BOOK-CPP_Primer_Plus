...
int main()
{
	...
	std::srand(static_cast<unsigned>(std::time(nullptr))); // random initializing of rand()
	...
	if (customers1 + customers2 > 0)
	{
		...
		cout << " average wait time: " << (double)(line_wait1 + line_wait2) / (served1 + served2) << " minutes\n";
	}
	else
		cout << "No customers!\n";

	cout << "\nThe average weight time is "	<< (double)(line_wait1 + line_wait2) / (served1 + served2) << " minutes\n";
	cout << "when there are " << perhour << " customers per hour\n";
