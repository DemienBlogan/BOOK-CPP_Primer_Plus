...
double LightsToAstroUnits(double lightYears);
...
int main()
{
	...
	double astonomicalUnits = LightsToAstroUnits(lightYears);

	cout << lightYears << " light years = " << astonomicalUnits << " astonomical units.\n";
...
double LightsToAstroUnits(double lightYears)
{
	double astonomicalUnits = lightYears * 63'240;
	return astonomicalUnits;
}